...
- 데몬이 건달을 고르면 데몬이 취해 **킬이 실패한다** (위키 샤발로스 예시: 둘 다 죽지 않는다).
...
| `demon` | 데몬 실제 생존(좀버얼 가짜 죽음 포함), 봉쇄 밤 아님, 음유시인 밤 아님 | 데몬 취함 → 그 밤 데몬 킬 실패 (§D4). 건달 → 악 |
...
20 demon (임프·Po·샤발로스·좀버얼·푸카·팡 구·비고르모르티스·노 다시·보르톡스)
...
## 9-A. 구현 뒤 검토에서 메운 구멍 (2026-09-02)

설계 시점에 놓쳤던 네 가지. 앞의 세 개는 가능한 세계를 잃던 **건전성** 문제였고,
마지막 하나는 불가능한 세계를 세던 문제였다.

1. **푸카는 밤1부터 고른다.** D1의 데몬 후보를 밤2부터로 두었는데 푸카만 밤1부터
   대상을 고른다 (킬이 밤2부터일 뿐이다). 밤1에 건달이 악해지는 세계를 잃고 있었다.
2. **기록 없는 선택자도 취한다.** `goodUnknown` 분기는 취함 표시를 하지 않는데,
   실제로는 누군가 한 명이 확실히 취한다. 수학자가 그 한 명을 셀 수 있어야 한다
   (`DemonScenario.goonUnknownDrunk` → 그 밤 최대값 +1, 관대한 방향).
3. **독살범은 건달을 중독시키지 못한다.** 고르는 순간 스스로 취하기 때문이다
   (공식 How to Run: "that player becomes drunk immediately"). D1의 `goonAbnormal`
   분기가 독살범의 독으로 성립하던 것은 유령 세계였다. 건달을 모든 밤의 독살 금지
   대상으로 두고, 같은 이유로 푸카의 관대 집합에서도 건달을 뺀다.
4. **대신은 캐릭터를 고른다.** 그래서 건달을 발동시키지 않고, 대신이 '건달'을 고른
   밤에는 건달이 조용히 취한다. 3번으로 `goonAbnormal`이 좁아지면서 이 분기가 없으면
   "정직한 기록된 선택자가 있는데 건달은 대신 때문에 취해 있던" 세계를 잃는다.

각 항목에는 그 수정이 없으면 실패하는 회귀 테스트가 있다 (`tests/solver/goon.test.ts`의
"검토에서 드러난 구멍").

**남은 관대한 근사**: 대신이 건달을 고른 밤의 분기는 대신이 실제로 효과를 냈는지
(멀쩡했는지) 따지지 않고 열린다. 불가능한 세계를 더 세는 방향이라 안전하다.
