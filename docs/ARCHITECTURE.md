...
- **사전 ≠ 커버리지.** 역할 사전(`ROLE_IDS`)은 3개 판본 72종 전부지만, 능력이 구현된 역할은
  `SOLVER_ROLES`뿐이다(현재 20종). 에디터에서는 72종 전부를 풀·정답 배치·거짓 정보 토큰으로
  쓸 수 있다.
- **모르는 역할은 세지 않고 거부한다.** `solve.ts`의 `validatePuzzle`이 *배정될 수 있는*
  역할 — 풀 안의 하수인, 주장 역할, 주정뱅이, 데몬 자리(임프 고정) — 을 모아 `SOLVER_ROLES`에
  없으면 던진다. 모르는 능력을 없는 셈 치고 세면 "유일해"가 거짓이 되기 때문이다.
  단지 풀에 들어 있을 뿐 배정되지 않는 역할(은둔자·첩자의 오등록 대상)은 통과시킨다 —
  보여줄 토큰의 이름일 뿐이라 능력 모델링이 필요 없다.
...
- 링크가 나오는 조건은 **배정되는 역할이 전부 `SOLVER_ROLES`에 있고, 그 배치가 유일해일 때**다.
  아직 구현되지 않은 역할을 쓴 문제는 편집·저장은 되지만 검증 단계에서 어떤 역할이 걸렸는지
  안내하고 링크를 내주지 않는다 (§4.2).
