...
export const SOLVER_ROLES: readonly RoleId[] = [
  ...
  "zombuul", // 임프 대체 데몬: 직전 낮 처형 사망이 있으면 안 깨어남 + 첫 죽음은 가짜 (등록상 사망, 비밀 생존)
  "pukka", // 임프 대체 데몬: 밤 n의 킬 = 밤 n-1의 중독 선택 — 선택·실행 두 밤의 멀쩡함 강제 (timeline이 밤별 독 가능 집합 계산)
  // ── SV ──
  ...
  "fanggu", // 임프 대체 데몬: 첫 외부인 킬이 점프가 된다 — 외부인이 팡 구가 되고 원래 팡 구가 죽는다 (1회). [+1 외부인]
...
export type InfoData =
  ...
  // 밤 n의 정보는 낮 n-1을 가리킨다. "예"는 미기록 투표/지명이 있을 수 있어 항상 성립하고
  // (관대한 방향), "아니오"가 그날 기록된 투표자/지명자의 등록을 제약한다.
  | { type: "flowergirl"; yes: boolean } // 어제 악마가 투표했는가
  | { type: "towncrier"; yes: boolean } // 어제 하수인이 지명했는가
...
export function eventDeadSeat(e: GameEvent): Seat | null {
  switch (e.type) {
    case "execution":
    case "death":
      return e.seat;
    case "slayerShot":
      return e.died ? e.target : null;
    case "virginTrigger":
      return e.nominator;
    case "nomination":
    case "vote":
