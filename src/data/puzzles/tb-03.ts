...
export default definePuzzle({
  ...
  playerCount: 7,
  nights: 2,
  rolePool: ["imp", "spy", "washerwoman", "investigator", "chef", "empath", "fortuneteller", "undertaker"],
  intro:
  ...
  hints: [
    "요리사 주장이 둘(C, E)인데 외지인이 없는 판이다 — 최소 한 명은 악역이다. 그리고 처형된 C가 임프였다면, 부정한 여인이 없는 이 스크립트에서 게임이 계속될 수 있었을까?",
  ...
  walkthrough: [
    ...
    "⑥ 파트너가 B라면 A·D·F·G가 모두 선·정직이다. 그러면 D의 수사관 정보로 B가 첩자, 즉 C가 임프가 된다. 하지만 C는 낮에 처형됐고 부정한 여인이 없는 이 판에서 임프 처형은 곧 게임 종료다 — 게임은 계속되고 있으므로 모순. 게다가 선한 장의사 A가 봤을 토큰도 '요리사'가 아니라 '임프'였을 것이다.",
