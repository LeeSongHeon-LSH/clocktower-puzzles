...
// 보통: 좀버얼의 가짜 죽음 — 처형한 그 사람이 지금도 악마다.
...
export default definePuzzle({
  ...
  walkthrough: [
    ...
    "⑧ 첫 죽음이 가짜인 악마는 좀버얼뿐이다. 좀버얼은 처형된 척 토큰을 넘기고 비밀리에 살아남는다. 죽음의 리듬도 그것을 받쳐 준다 — 좀버얼은 직전 낮에 처형이 있었으면 그 밤 깨어나지 않으므로 밤2가 조용했고, 낮2에 처형이 없자 밤3에 다시 손을 뻗어 A를 죽였다. 임프·포·샤발로스·노 다시라면 처형과 함께 정말로 죽어 게임이 끝났을 것이고, 보르톡스라면 처형 없이 지나간 낮2에 이미 악이 승리했을 것이다.",
    "⑨ 재구성: 좀버얼 D는 세탁부를 사칭하며 A가 진짜 요리사임을 확인해 주는 척했고, 그 덕에 마을은 요리사를 사칭한 하수인 E 대신 D를 처형했다 — 마을이 악마를 처형하고도 이기지 못한 이유가 그것이다. 지금 이 순간에도 악마는 D이고, 그 옆에서 E가 여전히 살아 있다.",
  ],
