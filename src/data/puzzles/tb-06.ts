...
export default definePuzzle({
  ...
  rolePool: [
    "imp", "scarletwoman",
    "ravenkeeper", "washerwoman", "investigator", "chef", "empath", "fortuneteller", "undertaker",
  ],
  intro:
    "7인 게임, 2일차 아침. 어제 낮, '수사관' F가 C와 G를 부정한 여인으로 지목했고 " +
    "마을은 요리사를 주장하던 G를 처형했다. 밤사이 A가 죽었다 — 그러나 까마귀지기였던 A는 " +
    "죽는 순간 눈을 떠 F의 역할을 확인했다: '부정한 여인'. " +
  ...
  questions: [
    ...
    { id: "scarletwoman", text: "부정한 여인은 누구인가?", answerSeats: [5] },
  ],
  hints: [
  ...
  walkthrough: [
    "① 외지인이 없는 판이므로 선한 플레이어는 전원 정직하다. A는 'F가 부정한 여인', F는 '수사관인데 C 또는 G가 부정한 여인'이라 주장한다 — 부정한 여인 토큰은 하나뿐이니 둘 다 선일 수 없다.",
    "② F가 정직한 수사관이라 가정하자. 그럼 A는 악역인데, A는 밤에 죽었으므로 A가 임프였다면 자살(스타 패스)이고 부정한 여인(C 또는 G)이 임프를 승계해야 한다. G라면 이미 처형돼 승계 불가 — 게임 종료 모순. C라면 C의 토큰이 임프가 되는데, 선한 점쟁이 D의 밤2 'A·C에 악마 없음'과 모순된다(죽은 A의 임프 토큰이든 승계한 C든 강제로 '있음'). A가 임프가 아니라 부정한 여인이라 해도 수사관 정보의 부정한 여인(C/G)과 이중이 되어 모순. 따라서 F가 악역이고, A의 유언은 참이다: F = 부정한 여인.",
    "③ 처형된 G가 임프였다면 부정한 여인 F가 승계해 게임이 계속됐을 수 있다 — 하지만 선한 장의사 E가 본 G의 토큰은 '요리사'다. G는 임프가 아니었다.",
