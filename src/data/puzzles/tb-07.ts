...
// 쉬움: 성결자 발동 — 낮에 일어난 사건 하나가 두 좌석을 한꺼번에 못박는다.
// 지명당한 자는 진짜 성결자이고 멀쩡했으며, 지명한 자는 마을 주민으로 등록됐다.
export default definePuzzle({
  ...
  intro:
    "7인 게임, 2일차 아침. 어제 낮 B가 A를 지명하자 A가 성결자임이 드러났고, " +
    "B는 그 자리에서 처형됐다. 그리고 밤사이 C가 죽었다. " +
    "남은 사람들이 각자 정보를 내놓았는데 — 초공감자를 주장하는 사람이 둘이다. " +
    "대본의 악마는 임프·좀버얼·팡 구 셋 중 하나다.",
  claims: [
  ...
  hints: [
    "밤 정보보다 낮에 일어난 일이 먼저다. 성결자의 능력이 실제로 발동했다는 것은 지명한 사람에 대해 무엇을 증명하는가?",
    "초공감자를 주장하는 사람이 둘(E, F)이다. 둘 중 한 명이 진짜라고 가정했을 때 그 수치가 다른 한 명의 정체와 앞뒤가 맞는지 따져 보라.",
  ],
  walkthrough: [
    "① 성결자의 능력이 발동했다는 것은 세 가지를 동시에 증명한다: 지명당한 A는 진짜 성결자이고, 그 순간 취하지도 중독되지도 않았으며, 지명한 B는 마을 주민으로 등록됐다. 이 대본에 첩자는 없으므로 B는 진짜 마을 주민 — 즉 A와 B는 둘 다 선하다.",
    ...
    "⑧ 악마의 종류도 이미 정해져 있었다. 좀버얼이라면 낮1에 처형 사망이 있었으므로 그 밤 아예 깨어나지 못해 C가 죽을 수 없다. 팡 구라면 외지인이 한 명 늘어 7인 판에 외지인 자리가 생기는데, 아무도 외지인을 주장하지 않았고 정체를 감출 주정뱅이도 대본에 없다. 남는 것은 임프뿐이다.",
    "⑨ 재구성: 임프 D는 수사관을 사칭하며 선한 A와 F에게 혐의를 씌웠고, 탕녀 E는 초공감자를 사칭했다. 마을은 정보가 아니라 낮의 사고로 진실에 닿았다 — 성결자에게 손을 댄 B가 죽는 순간, 이미 두 사람의 결백이 증명되어 있었다. 선한 요리사 C의 '인접 악역 쌍 1'도 나란히 앉은 D와 E를 가리키고 있었다.",
  ],
